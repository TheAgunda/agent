...
function summarizeCall(name: string, args: Record<string, unknown>): string {
  const entries = Object.entries(args)
    .map(
      ([k, v]) =>
        `${k}=${typeof v === "string" ? truncate(v, 120) : JSON.stringify(v)}`,
    )
    .join(", ");
  return `${name}(${entries})`;
}
...
export class PermissionManager {
  ...
  evaluate(
    ...
    if (
      targetPath &&
      tool.riskLevel !== "read" &&
      !this.config.allowEditOutsideProject
    ) {
    ...
    if (tool.name === "run_command") {
      ...
      const blocked = this.config.blockedCommands.find((pattern) =>
    ...
    if (this.config.mode === "auto") {
      const alwaysConfirm = this.config.requireConfirmationFor.includes(
        tool.name,
      );
      return {
        allowed: true,
