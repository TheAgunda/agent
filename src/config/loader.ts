...
import { AgentConfig, DEFAULT_CONFIG } from './schema.js';
...
function readYamlIfExists(filePath: string): Record<string, unknown> | null {
  ...
  if (typeof parsed !== 'object') {
    throw new Error(`Config file ${filePath} must contain a YAML object at the top level.`);
  }
...
function applyEnvOverrides(config: AgentConfig): AgentConfig {
  ...
  if (env.AGENT_MODEL_PROVIDER) next.model.provider = env.AGENT_MODEL_PROVIDER as AgentConfig['model']['provider'];
  ...
  if (env.OPENAI_API_KEY && !next.model.apiKey && next.model.provider === 'openai') {
    next.model.apiKey = env.OPENAI_API_KEY;
  }
  if (env.AGENT_PERMISSIONS_MODE) {
    next.permissions.mode = env.AGENT_PERMISSIONS_MODE as AgentConfig['permissions']['mode'];
  }
  return next;
}
...
export function validateConfig(config: AgentConfig): string[] {
  ...
  const validProviders = ['ollama', 'llamacpp', 'vllm', 'openai-compatible', 'openai'];
  if (config.model?.provider && !validProviders.includes(config.model.provider)) {
    problems.push(`model.provider must be one of ${validProviders.join(', ')}`);
  }
  if (config.agent?.maxIterations !== undefined && config.agent.maxIterations <= 0) {
...
export function loadConfig(options: LoadConfigOptions): AgentConfig {
  ...
  const projectOverrides = readYamlIfExists(projectConfigPath(options.projectRoot));

  let config = deepMerge(DEFAULT_CONFIG, (globalOverrides as Partial<AgentConfig>) ?? undefined);
  config = deepMerge(config, (projectOverrides as Partial<AgentConfig>) ?? undefined);
...
export function writeProjectConfig(projectRoot: string, config: AgentConfig): void {
  const dir = projectAgentDir(projectRoot);
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(projectConfigPath(projectRoot), yaml.dump(config, { lineWidth: 100 }), 'utf-8');
}

export function writeGlobalConfig(config: Partial<AgentConfig>): void {
  fs.mkdirSync(GLOBAL_CONFIG_DIR, { recursive: true });
  fs.writeFileSync(GLOBAL_CONFIG_PATH, yaml.dump(config, { lineWidth: 100 }), 'utf-8');
}
