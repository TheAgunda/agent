...
export class Planner {
  ...
  save(plan: Plan): void {
    ...
    fs.writeFileSync(planPath(this.projectRoot, plan.id), JSON.stringify(plan, null, 2), 'utf-8');
    fs.writeFileSync(path.join(plansDir(this.projectRoot), 'latest.json'), JSON.stringify({ planId: plan.id }), 'utf-8');
  }
  ...
  updateTaskStatus(plan: Plan, taskId: string, status: PlanTask['status'], notes?: string): Plan {
  ...
  renderChecklist(plan: Plan): string {
    ...
    return plan.tasks.map((t) => `${marks[t.status]} ${t.description}`).join('\n');
  }
}
