...
export const gitStatusTool: Tool = {
  definition: {
    name: 'git_status',
    description: 'Show the current git status (staged, unstaged, untracked files, current branch).',
  ...
  async execute(_args, ctx: ToolContext) {
    try {
      ...
      return fail(`Not a git repository or git error: ${(err as Error).message}`);
    }
  },
};
...
export const gitDiffTool: Tool = {
  definition: {
    name: 'git_diff',
    description: 'Show the diff of uncommitted changes, optionally for a specific file.',
    parameters: {
      type: 'object',
  ...
  async execute(args, ctx: ToolContext) {
    try {
      const git = client(ctx.projectRoot);
      const diff = args.path ? await git.diff([String(args.path)]) : await git.diff();
...
export const gitLogTool: Tool = {
  definition: {
    ...
    parameters: {
      type: 'object',
      properties: { maxCount: { type: 'number', description: 'Max commits to show (default 10).' } },
    },
    riskLevel: 'read',
  },
  async execute(args, ctx: ToolContext) {
    try {
      ...
      const lines = log.all.map((c) => `${c.hash.slice(0, 8)} ${c.date.slice(0, 10)} ${c.message}`);
      return ok(lines.join('\n') || '(no commits yet)');
    } catch (err) {
...
export const gitCommitTool: Tool = {
  definition: {
    name: 'git_commit',
    description: 'Stage all tracked changes and create a commit with the given message. Never force-pushes or rewrites history.',
    parameters: {
      type: 'object',
  ...
  async execute(args, ctx: ToolContext) {
    try {
      const git = client(ctx.projectRoot);
      const status = await git.status();
      if (status.files.length === 0) return ok('Nothing to commit — working tree clean.');
      await git.add(['-A']);
      const message = String(args.message ?? 'Automated commit');
      const result = await git.commit(message);
      return ok(`Committed ${result.summary.changes} change(s) as ${result.commit}: "${message}"`, result);
    } catch (err) {
