...
export class ContextManager {
  ...
  markTouched(relPath: string): void {
    this.recentlyTouched = [
      relPath,
      ...this.recentlyTouched.filter((p) => p !== relPath),
    ].slice(0, 20);
  }
  ...
  buildContext(taskDescription: string): BuiltContext {
    ...
    const summaryTokens =
      estimateTokens(summaryBlock) + estimateTokens(treeBlock);
    const fileBudget = Math.max(
      0,
      this.options.contextLimit - summaryTokens - 500,
    );
