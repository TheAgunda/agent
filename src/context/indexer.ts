...
let cachedFileList: { root: string; ignore: string[]; files: string[] } | null = null;
...
/**
 * Recursively list all non-ignored files in the project, relative to `projectRoot`.
 * Cached per (root, ignore-list) pair for the life of the process since re-walking
 * large trees on every tool call would be wasteful; callers needing a fresh view
 * (e.g. after big scaffolding operations) should call `invalidateFileListCache()`.
 */
export function listProjectFiles(projectRoot: string, extraIgnores: string[] = []): string[] {
  const cacheKey = JSON.stringify(extraIgnores);
  if (cachedFileList && cachedFileList.root === projectRoot && JSON.stringify(cachedFileList.ignore) === cacheKey) {
...
function detectFrameworksAndPackageManagers(projectRoot: string, files: Set<string>): {
  frameworks: string[];
  ...
  for (const signal of FRAMEWORK_SIGNALS) {
    if (signal.dependency && deps[signal.dependency]) frameworks.add(signal.framework);
...
function detectCommands(projectRoot: string): { test?: string; lint?: string; format?: string } {
  const pkgPath = path.join(projectRoot, 'package.json');
  if (fs.existsSync(pkgPath)) {
    const pkg = readJsonSafe(pkgPath);
...
export function buildProjectInfo(projectRoot: string, extraIgnores: string[] = []): ProjectInfo {
  const files = listProjectFiles(projectRoot, extraIgnores);
  const fileSet = new Set(files.map((f) => path.basename(f)));
  const { frameworks, packageManagers } = detectFrameworksAndPackageManagers(projectRoot, fileSet);
