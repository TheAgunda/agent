...
function scoreFile(relPath: string, queryTokens: string[], recentlyTouched: Set<string>): FileContextEntry {
  ...
  return { path: relPath, relevance: score, reason: reasons.join(', ') || 'baseline' };
}
...
/**
 * Pick the most relevant files for a given task within a token budget, instead of
 * naively dumping the whole repository into the prompt.
 */
export function selectRelevantFiles(
  projectRoot: string,
  taskDescription: string,
  options: { tokenBudget: number; recentlyTouched?: string[]; extraIgnores?: string[] }
): SelectionResult {
  ...
  for (const entry of scored) {
    ...
    if (usedTokens + approxTokens > options.tokenBudget && selected.length > 0) continue;
    selected.push(entry);
    usedTokens += approxTokens;
  }

  return { selected, totalCandidates: files.length, estimatedTokens: usedTokens };
}
